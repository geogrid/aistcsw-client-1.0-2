...
//        $this->_buildAndExecuteProxyResponse();
    ...
    protected static function _getStringFromRequestType($type)
    {
        ...
        elseif ($type === self::REQUEST_METHOD_GET)
            $name = "GET"; elseif ($type === self::REQUEST_METHOD_PUT)
            $name = "PUT"; elseif ($type === self::REQUEST_METHOD_DELETE)
            $name = "DELETE"; else
    ...
    protected function _loadRoute()
    {
        ...
        $regex = "/^.+$basename\//";
        $route = preg_replace($regex, '', $_SERVER['REQUEST_URI']);
        $this->_route = $route;
    }
    ...
    protected function _loadRequestMethod()
    {
        ...
        elseif ($method == "post")
            $this->_requestMethod = self::REQUEST_METHOD_POST; elseif ($method == "put")
            $this->_requestMethod = self::REQUEST_METHOD_PUT; elseif ($method == "delete")
            $this->_requestMethod = self::REQUEST_METHOD_DELETE; else
    ...
    protected function _makeCurlRequest($url)
    {
        ...
        $length = $info['download_content_length'] == -1 ? $info['size_download'] : $info['download_content_length'];
        header('Content-Length:' . $length);
        header('Content-Disposition: attachment; filename="' . basename($info['url']) . '"');
        fseek($fp, 0);
        while (!feof($fp))
            echo fread($fp, 1024);
        fclose($fp);
    ...
    {
        if (!array_isset($meta['wrapper_data']))
    ...
    protected function _parseResponse()
    {
        ...
        elseif ($break_2 && $break_1 === FALSE) $break = $break_2; elseif ($break_1 < $break_2) $break = $break_1; else $break = $break_2;
        ...
        $this->_responseBody    = preg_replace('/^(\r\n|\n)+/s', '', substr($this->_rawResponse, $break));
        $this->_responseBody    = preg_replace('/(\r\n|\n)+$/s', '', $this->_responseBody);
    }
    ...
    protected function _generateProxyResponseHeaders()
    {
        foreach ($this->_responseHeaders as $name => $value) {
            if ($name != 'status' and $name != "Transfer-Encoding")
                header("$name: $value");
        }
    }
